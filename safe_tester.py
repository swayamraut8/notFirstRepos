...
def show_dashboard():
    ...
    # Header bar
    header_bar = tk.Frame(dashboard_frame, bg="#62daff")
    header_bar.pack(fill=tk.X, pady=10)

    wallet_balance_label = tk.Label(header_bar, text=f"Wallet Balance: ₹{current_user[8]:.2f}", font=("Arial", 14), bg="#62daff", fg="white")
    wallet_balance_label.pack(side=tk.RIGHT, padx=10)

    # Left sidebar
    sidebar_frame = tk.Frame(dashboard_frame, bg="#62daff")
    sidebar_frame.pack(side=tk.LEFT, fill=tk.BOTH)

    # Book Parking lot button
    book_parking_button = tk.Button(sidebar_frame, text="Book Parking Lot", font=("Arial", 12), bg="#ffeb3b", fg="#000000", command=lambda: show_book_parking(content_frame))
    book_parking_button.pack(pady=10, padx=10, fill=tk.X)

    # Booking History button
    booking_history_button = tk.Button(sidebar_frame, text="Booking History", font=("Arial", 12), bg="#ffeb3b", fg="#000000", command=lambda: show_booking_history(content_frame))
    booking_history_button.pack(pady=10, padx=10, fill=tk.X)

    # Load Balance button
    load_balance_button = tk.Button(sidebar_frame, text="Load Balance", font=("Arial", 12), bg="#ffeb3b", fg="#000000", command=show_load_balance)
    load_balance_button.pack(pady=10, padx=10, fill=tk.X)

    # Profile button
    profile_button = tk.Button(sidebar_frame, text="Profile", font=("Arial", 12), bg="#ffeb3b", fg="#000000", command=show_profile)
    profile_button.pack(pady=10, padx=10, fill=tk.X)

    # Logout button
    logout_button = tk.Button(sidebar_frame, text="Logout", font=("Arial", 12), bg="#ffeb3b", fg="#000000", command=logout)
    logout_button.pack(pady=10, padx=10, fill=tk.X)

    # Main content area
    content_frame = tk.Frame(dashboard_frame, bg="#ffe085")
...
def show_book_parking(content_frame):  # Add content_frame as a parameter
    ...
    # Select button
    select_button = tk.Button(content_frame, text="Select", fg="black" ,bg="yellow" ,font=("Arial", 14), command=lambda: show_parking_spots(content_frame, parking_lot_dropdown.get()))  # Pass content_frame here
...
def show_parking_spots(content_frame, location):  # Add content_frame as a parameter
    ...
    if parking_lot:
        ...
        for i in range(total_spots):
            spot_number = i + 1
            spot_button = tk.Button(spots_frame, text=str(spot_number), font=("Arial", 14), bg="light green", width=4, height=2, command=lambda spot=spot_number: select_spot(content_frame, spot))
        ...
        # Book button
        book_button = tk.Button(content_frame, text="Book", bg="yellow",fg="black",font=("Arial", 14), command=book_spot)
...
def select_spot(content_frame, spot_number):  # Add content_frame as a parameter
    ...
    if selected_spot:
        ...
        print(content_frame.winfo_children())
...
def book_spot():
    ...
    if selected_spot:
        # Ensure content_frame has enough child widgets
        if len(content_frame.winfo_children()) > 1:
            ...
            messagebox.showerror("Widget Error", "content_frame does not have enough child widgets.")
    else:
        messagebox.showerror("No Spot Selected", "Please select an available spot to book.")


def show_booking_history(content_frame):  
    # Clear the content frame
    for widget in content_frame.winfo_children():
        widget.destroy()

    # Create a treeview to display the booking history
    booking_history_tree = ttk.Treeview(content_frame, columns=("Booking Time", "Exit Time", "Parking Lot", "Spot", "Duration", "Amount Paid"), show="headings")
    booking_history_tree.heading("Booking Time", text="Booking ID")
    booking_history_tree.heading("Exit Time", text="Entry Time")
    booking_history_tree.heading("Parking Lot", text="Exit Time")
    booking_history_tree.heading("Spot", text="Parking Lot")
    booking_history_tree.heading("Duration", text="Spot")
    booking_history_tree.heading("Amount Paid", text="Duration (Hours)")
    booking_history_tree.pack(pady=10)

    # Fetch the user's booking history from the database
    c.execute("SELECT b.booking_id, b.booking_time, b.exit_time, p.location, b.spot_number, b.duration_hours, b.amount_paid, b.additional_amount FROM Bookings b JOIN Parking_Lots p ON b.parking_lot_id = p.parking_lot_id WHERE b.user_id = ?", (current_user[0],))
    bookings = c.fetchall()

    # Insert the booking data into the treeview
    for booking in bookings:
        booking_id, booking_time, exit_time, location, spot_number, duration_hours, amount_paid, additional_amount = booking
        booking_history_tree.insert("", tk.END, values=(booking_id, booking_time, exit_time, location, spot_number, duration_hours, f"₹{amount_paid:.2f}"))

    # Exit button
    exit_button = ttk.Button(content_frame, text="Exit", command=lambda: exit_selected_booking(booking_history_tree))
    exit_button.config(style='Yellow.TButton')  # Applying the custom style
    exit_button.pack(pady=5)

    # View Invoice button
    invoice_button = ttk.Button(content_frame, text="View Invoice", command=lambda: view_selected_invoice(booking_history_tree))
    invoice_button.config(style='Yellow.TButton')  # Applying the custom style
    invoice_button.pack(pady=5)

    # Define the custom style for yellow buttons
    style = ttk.Style()
    style.configure('Yellow.TButton', foreground='black', background='yellow', font=("Arial", 14), width=20)



def exit_selected_booking(booking_history_tree):
    # Get the selected item from the treeview
    selected_item = booking_history_tree.selection()
    if selected_item:
        # Get the booking ID from the selected item
        booking_id = booking_history_tree.item(selected_item)['values'][0]
        # Call the exit parking function with the booking ID
        exit_parking(None, booking_id)  # Passing None for content_frame as it's not needed
...
def view_selected_invoice(booking_history_tree):
    # Get the selected item from the treeview
    selected_item = booking_history_tree.selection()
    if selected_item:
        # Get the booking ID from the selected item
        booking_id = booking_history_tree.item(selected_item)['values'][0]
        # Call the view invoice function with the booking ID
        view_invoice(None, booking_id)  # Passing None for content_frame as it's not needed
...
def show_load_balance():
    ...
    # Add balance button
    add_balance_button = tk.Button(content_frame, text="Add Balance", bg="yellow" , fg="black" ,font=("Arial", 14), command=lambda: add_balance(amount_entry.get()))
...
root.configure(bg="#ffe085")  # Set background color for the root window

# Heading
heading_label = tk.Label(root, text="Parking Lot Management System", font=("Arial", 24, "bold"), bg="#3f51b5", fg="white")
heading_label.pack(pady=(20, 10))

# Login frame
login_frame = tk.Frame(root, bg="#f0f0f0")  # Set background color for the login frame
login_frame.pack(pady=20)

# Labels and entries in the login frame
username_label = tk.Label(login_frame, text="Username:", font=("Arial", 14), bg="#f0f0f0")
username_label.grid(row=0, column=0, padx=10, pady=10)

username_entry = tk.Entry(login_frame, font=("Arial", 14), bg="white")  # Set entry background color to white
username_entry.grid(row=0, column=1, padx=10, pady=10)

password_label = tk.Label(login_frame, text="Password:", font=("Arial", 14), bg="#f0f0f0")
password_label.grid(row=1, column=0, padx=10, pady=10)

password_entry = tk.Entry(login_frame, font=("Arial", 14), show="*", bg="white")  # Set entry background color to white
password_entry.grid(row=1, column=1, padx=10, pady=10)

login_button = tk.Button(login_frame, text="Login", font=("Arial", 14), bg="#ffeb3b", fg="#000000", command=login)  # Yellow button with black text
login_button.grid(row=2, column=0, columnspan=2, pady=10, padx=10, sticky="we")  # Added padx and sticky options

# Main frame
main_frame = tk.Frame(root, bg="#ffe085")  # Set background color for the main frame
